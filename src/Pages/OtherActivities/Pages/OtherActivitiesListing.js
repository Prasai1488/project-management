...
import { otherActivitiesEditSuccess } from "../Redux/otherActivitiesSlice";
import { getNext } from "../Redux/thunk";

const OtherActivitiesListing = ({ dispatch, setOtherActivitiesModal, postsPerPage }) => {
  const listRef = useRef(null);

  const next = useSelector((state) => state.otherActivities.next);
  const loadingNext = useSelector((state) => state.otherActivities.loadingNext);
  ...
  const handleEdit = (id) => {
    dispatch(otherActivitiesEditSuccess(id));
...
export default OtherActivitiesListing;
